...
### 脑裂问题
#### 什么是脑裂
脑裂就是在哨兵环境中，客户端与redis master的网络正常，但哨兵与master的网络出现问题，  
导致哨兵重新在剩下的slave中进行master选举，这时由与主从复制策略，新master负责写，但得  
不到客户端的数据，slave负责读但同步的是新master的数据，客户端依然与旧的master进行通信  
，导致在一个系统中出现两个master，这种成为脑裂。

#### 问题如何解决
![](./images/redis-all/redis脑裂问题解决方案1.jpg)
